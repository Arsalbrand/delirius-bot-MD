...
module.exports = handler = async (mek, conn, map) => {
			...
			options.adReply
				? (content.contextInfo = {
						externalAdReply: {
							...
							body: config.namebot,
							thumbnail: await conn.getBuffer("https://i.postimg.cc/RZfPWT9G/descarga-1.jpg"),
  ...
  if(isGroup){
	...
	// [ Response ]
	global.respon = {
	  wait: "Espere un minuto, su solicitud está siendo procesada..",
	  success: "Listo ✓",
	  error:{
	  ...
	  private: "Este comando se puede usar en chat privado",
	  admin: "¡Este comando es solo para administradores!",
	  bAdmin: "¡El bot necesita administrador!",
	  owner: "Este comando solo puede ser usado por el creador"
	}
	...
	if(options.isQuoted && !msg.quoted) {
	  ...
	  return msg.reply(`Lo siento *${conn.getName(sender)}*, Comando *${cmdName}* siendo deshabilitado por _*Propietario!!*_\n_*Razón : ${alasan || 'Ninguno'}*_`)
	}
