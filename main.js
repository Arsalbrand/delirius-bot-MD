...
async function start(){
  ...
  console.log(color(figlet.textSync('@delirius_bot', {font: 'Standard',horizontalLayout: 'default',vertivalLayout: 'default',width: 80,whitespaceBreak: false }), 'red'))
	console.log(color(figlet.textSync('@darlyn-', {font: 'Caligraphy2', horizontalLayout: 'default',vertivalLayout: 'default', width: 80, whitespaceBreak: false}), 'brown'))
  console.log(color('------------------------------------------------------------------------', 'white'))
  console.log(color('[CREADOR]', 'aqua'), color(config.author, 'magenta'))
  console.log(color('[BOT]', 'aqua'), color('¡BOT ahora está en línea!', 'magenta'))
  console.log(color('[VERSION]', 'aqua'), color(`${version}`, 'magenta'))
    
  const conn = Baileys({
    printQRInTerminal: true,
		auth: state,
		browser: ['@delirius_bot Darlyn', "Chrome", "1.0.0"],
  ...
  const decodeJid = (jid) => {
	...
	conn.ev.on("connection.update", async (ktl) => {
	  ...
	  if (connection == "connecting") console.log(chalk.cyan('Conectándose al bot de WhatsApp...'))
	  if (connection) {
	    if (connection != "connecting") 
	      console.log(chalk.yellow("Coneccion: " + connection))
	  }
	  if (connection == "open") {
	    console.log(chalk.yellow("Conectado exitosamente a whatsapp"))
	    conn.sendMessage(config.owner[0],{text: "*¡El bot ahora está en línea!*"})
	  }
	  if (connection === "close") {
			...
			if (reason === DisconnectReason.badSession) {
				...
				console.log(chalk.yellow("Se requiere reinicio, reiniciando..."));
				start();
	...
	conn.ev.on("call", async (senku) => {
	  ...
	  for(let sen of senku){
	    if(sen.isGroup == false){
	      if(sen.status == "offer"){
	        teks = `*${conn.user.name}* no puedo recibir llamadas ${sen.isVideo ? `video` : `suara`}.¡Lo siento, serás bloqueado! Si fue un accidente, póngase en contacto con el propietario!\n\nOwner :\n${config.owner.map((a) => `${a.split(`@`)[0]} | ${conn.getName(a).includes("+62") ? "No Detect" : conn.getName(a)
							}`).join("\n")}`
