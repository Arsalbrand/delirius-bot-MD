  ...
  require("moment").locale("id");
	...
	for (let i of participant) {
	  try {
	    ...
	    ppimg = "https://i.postimg.cc/Dz3FV095/descarga-2.jpg"
	  }
...
const antiluar = async(conn, msg) => {
  ...
  let local = localOnly.includes(msg.id);
	...
	for (let i of participant) {
	  if (msg.action == "add"){
	    if(!local && i.startsWith('999') || i.startsWith('0') || i.startsWith('212') || i.startsWith('91') || i.startsWith('92') || i.startsWith('299') || i.startsWith('0') || i.startsWith('0') || i.startsWith('7')) {
	     let luar = `Lo siento @${i.split("@")[0]}⁩, en este grupo no se permite el codigo de tu pais*`
