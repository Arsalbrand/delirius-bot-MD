	  ...
	  try {
	    switch(cmdNya){
	      case "join":
	        ...
	        txt = "Invitacion a chat de grupo\n\n"
	        txt += "• Usuario : @" + msg.sender.split("@")[0] + "\n"
	        txt += "• De : " + msg.from + "\n"
	        txt += "• Link :" + q
          reply("Listo, espera a que el propietario acepte la invitacion")
          return conn.sendMessage(config.owner[0], {
                 text: txt,
                 footer: global.footer,
                 buttons: [ 
                     { buttonId: `.accgc ${q} ${msg.from} ${msg.key.id}`, buttonText: { displayText: 'Aceptar' }, type: 1 },
                     { buttonId: `.rejectgc ${msg.from} ${msg.key.id}`, buttonText: { displayText: 'Rechazar' }, type: 1 }
                   ],
                  headerType: 1
	      ...
	      case "accgc":
	        ...
	        const rex1 = /chat.whatsapp.com\/([\w\d]*)/g;
  	    	lap = 'La invitacion a sido aceptado por el propietario.\n\n'
          lap += "Detalles"
	      ...
	      case "rejectgc":
	        if(!own)return reply(respon.owner)
	        lap = 'El grupo a sido rechazado por el propietario\n\n'
          lap += "Detalles"
          lap +=  " " + msg.quoted.message.buttonsMessage.contentText.split("Invitación para unirse al grupo de WhatsApp")[1]
          ...
          conn.sendMessage(args[0], { text: `El propietario del bot si acepto la invitacion.` }, { quoted: w });
          await msg.reply("Listo.")
	       break
	    }
