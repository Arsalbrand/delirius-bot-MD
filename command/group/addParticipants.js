module.exports = {
  name: "add",
	alias: ["add"],
	category: "group",
	desc: "Agregar miembros al grupo",
	isGroup: true,
	isBotAdmin: true,
	isAdmin: true,
	async run({msg, conn},{q, prefix}){
	  add = q ? q : msg.quoted ? msg.quoted : false;
	  if(!add) throw `Ejemplo : ${prefix + msg.command} 12022002000`
	  let chunk = [];
	  try {
	  	...
	  	for (let i of participant) {
	  	  if (!global.statParticipant && !cek.participants.includes(i)){
	  	    const code = await conn.groupInviteCode(msg.from);
		  		await msg.reply(" El número @" + i.split("@")[0] + " que agregó es privado, invitando al usuario...", {withTag: true,});
