...
module.exports = {
  ...
  async run({conn, msg},{args, q}){
    ...
    switch(command){
      ...
      case "addtoxic":
        if(!q) throw "Introduce una palabra prohibida"
          try {
            ...
            if(word[from].kata.includes(q)) return reply(`La palabra ${q} ya está en la lista negra`)
            word[from].kata.push(q)
            await fs.writeFileSync('./lib/database/toxic.json',JSON.stringify(word))
            reply(`La palabra ${q} se ingresó con éxito`)
     ...
     case "deltoxic":
       ...
       try {
         const word = JSON.parse(fs.readFileSync('./lib/database/toxic.json'))
         if(!word[from].kata.includes(q)) return msg.reply(`La palabra ${q} no está en la lista negra de palabras`)
         word[from].kata.splice(word[from].kata.indexOf(q), 1)
         await fs.writeFileSync('./lib/database/toxic.json',JSON.stringify(word))
         reply(`La palabra ${q} se eliminó con éxito`)
       } catch (e){
         global.error(command, e, msg)
       }
       break
    ...
    case "listtoxic":
      ...
      txt = "*Lista negra en el grupo : " + data.subject + "*\n\n"
